...
describe('keyPasswd', () => {
  ...
  it('remove password from current key', {
    timeout: 10_000
  }, async () => {
  ...
  it('update password for current key', {
    timeout: 10_000
  }, async () => {
    await writeFile(join(dir, 'pwd'), 'new-password');
    await keyPasswd().repository(join(dir, 'repository')).password('password').newPasswordFile(join(dir, 'pwd')).run();
